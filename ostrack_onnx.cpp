...
std::vector<float> OSTrack::hann(int sz) {
...
std::vector<Ort::Value>  OSTrack::transform(const cv::Mat &mat_z, const cv::Mat &mat_x)
{
...
// calculate bbox
void OSTrack::cal_bbox(std::vector<Ort::Value> &output_tensors, DrBBox &pred_box, float &max_score, float resize_factor) {
...
Ort::Value OSTrack::create_tensor(const cv::Mat &mat, 
...
cv::Mat OSTrack::normalize(const cv::Mat &mat, float mean, float scale)
{
  cv::Mat matf;
...
cv::Mat OSTrack::normalize(const cv::Mat &mat, const float mean[3], const float scale[3])
{
  cv::Mat mat_copy;
...
void OSTrack::normalize(const cv::Mat &inmat, cv::Mat &outmat, float mean, float scale)
{
  outmat = this->normalize(inmat, mean, scale);
}

void OSTrack::normalize_inplace(cv::Mat &mat_inplace, float mean, float scale)
{
  if (mat_inplace.type() != CV_32FC3) mat_inplace.convertTo(mat_inplace, CV_32FC3);
  this->normalize(mat_inplace, mat_inplace, mean, scale);
}

void OSTrack::normalize_inplace(cv::Mat &mat_inplace, const float mean[3], const float scale[3])
{
  if (mat_inplace.type() != CV_32FC3) mat_inplace.convertTo(mat_inplace, CV_32FC3);
...
OSTrack::OSTrack(const std::string &_onnx_path, unsigned int _num_threads) :
